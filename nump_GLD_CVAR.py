...
def gradient_langevin_dynamics_iteration(num_iterations, epsilon, step_size, noise_scale, init_x, init_y, theta):
    ...
    for iteration in range(num_iterations):
        ...
        grad_x, grad_y = gradient_g(x+5-theta[0], y-4.5-theta[1])
...
def gradient_tilde_g(theta, z):
    x, y = z
    grad_g_x, grad_g_y = gradient_g(x +5- theta[0], y-4.5 - theta[1])
...
def f(theta, Y_sample):
    return (Y_sample[1])**2+(Y_sample[0])**2
...
def expected_gradient(theta, beta, Y_samples, lambda_, delta, gamma):
    ...
    for Y_sample in Y_samples:
        ...
        grad_f = gradient_f(theta, Y_sample)
        compute_expressions=compute_expression(theta, Y_sample, lambda_, Y_samples)
        #print('compute_expression:',compute_expressions)
        #print('f_value:',f_value)
        #print('h_prime:',h_prime)
        #print('h:',h(f_value - beta, gamma))

        gradient_sum_theta += h_prime * grad_f + compute_expressions * h(f_value - beta, gamma)

    expected_grad_theta = gradient_sum_theta / (delta*len(Y_samples))
    return expected_grad_theta

def expected_gradient_beta(theta, beta, Y_samples, lambda_, delta, gamma):
    gradient_sum_beta = 0
    for Y_sample in Y_samples:
        f_value = f(theta, Y_sample)
        h_prime = derivative_h(f_value - beta, gamma)
    ...
    return expected_grad_beta

# Gradient descent algorithm
def gradient_descent(theta_init, beta_init, step_size, Y_samples, lambda_, delta, gamma,beta_num_itera):
    theta = theta_init
    beta = beta_init
    #for _ in range(num_iterations):
    grad_phi_theta = expected_gradient(theta, beta, Y_samples, lambda_, delta, gamma)
    theta = theta - step_size * grad_phi_theta
    return theta

def CVAR_delta(theta, Y_samples, lambda_, delta, gamma):
    #return 0
    beta2=0
    h1_sum=0
    sample_f_values=[]
    ...
    beta2=np.nanquantile(sample_f_values, 1-delta)    
    print('beta2', beta2)
    for Y_sample in Y_samples:
        f_value = f(theta, Y_sample)
        h1=h(f_value - beta2, gamma)
        h1_sum+=h1
...
for num_iterations in range(20):
    ...
    grad_phi_theta1 =expected_gradient(theta,beta,samples,epsilon,delta,gamma)
    #grad_phi_beta1 =expected_gradient_beta(theta,beta,samples,epsilon,delta,gamma)
    samples_phi_grad.append(LA.norm(grad_phi_theta1,2))
    print('gradient_phi_theta:',grad_phi_theta1)
    print('theta_update:',theta)
